...
class LSM{
    ...
    void do_merge(){
        ...
        for(int i=_activeRun;i<_num_runs;i++){
            Skiplist *run=new Skiplist(INT16_MIN,INT16_MAX);
    ...
    void insert_key(string key){
        ...
        if(_activeRun>=_num_runs)
            do_merge();
        C_0[_activeRun]->insertkey(key,value);
        filters[_activeRun]->add(&key,sizeof(int));
    }
    ...
    bool lookup(string key){
        ...
        for(int i=_activeRun;i>=0;i--){
            if(key<C_0[i]->get_min() || key>C_0[i]->get_max() || filters[i]->maycontain(&key,sizeof(int)))
                continue;
        ...
        for(int i=_activeRun;i>=0;i--){
            ...
            if(curelts.size()!=0){
                ...
                for(int c=0;c<curelts.size();c++){
                    int dummy=ht.putifempty(curelts[c].first,curelts[c].second);
                    if(!dummy && curelts[c].second!=v_tombstone)
            ...
            for(int r=disklevels[j]->_activerun-1;r>=0;r--){
                ...
                if(i2-i1!=0){
                    ...
                    for(long m=i1;m<i2;m++){
                        ...
                        int dummy=ht.putifempty(kv.first,kv.second);
                        if(!dummy && kv.second!=v_tombstone)
